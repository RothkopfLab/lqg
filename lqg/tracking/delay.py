...
def delay_system(spec, delay):
    ...
    A = jnp.stack(
        [
            linalg.block_diag(A, jnp.diag(jnp.zeros(d * delay)))
            + jnp.diag(jnp.ones(d * delay), k=-d)
            for A in spec.A
        ]
    )
    B = jnp.stack([jnp.vstack([B] + [jnp.zeros_like(B)] * delay) for B in spec.B])
    F = jnp.stack(
        [jnp.hstack([jnp.zeros((F.shape[0], F.shape[1] * delay)), F]) for F in spec.F]
    )

    V = jnp.stack(
        [linalg.block_diag(V, jnp.diag(jnp.zeros(d * delay))) for V in spec.V]
    )
    ...
    return LQGSpec(
        A=A, B=B, F=F, V=V, W=spec.W, Q=Q, R=spec.R, q=q, Qf=Qf, qf=qf, P=P, r=r
    )
