...
from invlqg.model import System, Dynamics, Actor, SystemSpec


def DimModel(dim=3, process_noise=1.0, motor_noise=0.5,
    ...
    dyn = Dynamics(A=A, B=B, H=H, V=V)
    act = Actor(A=A, B=B, H=H, V=V, W=W, Q=Q, R=R)

    return SystemSpec(actor=act, dynamics=dyn)


def OneDimModel(process_noise=1.0, motor_noise=0.5,
                sigma=6.0, prop_noise=6.0, c=1.0, dt=1. / 60):
    return DimModel(dim=1, process_noise=process_noise,
...
class DiffModel(System):
    def __init__(self, dim=1, process_noise=1.0, motor_noise=0.5,
        ...
        # observation model
        H = linalg.block_diag(*[jnp.array([[1., -1.]])] * dim)
